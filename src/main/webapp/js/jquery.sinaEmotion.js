...
(function($) {
	...
	var showFacePage = function(page) {
		...
		for (var i = page * rows, l = faces.length; i < l
				&& i < (page + 1) * rows; ++i) {
			face = faces[i];
			html += '<li class="item"><a href="#" class="face"><img style="width: 22px" class="sina-emotion" src="'
					+ face.icon + '" alt="' + face.phrase + '" /></a></li>';//点击表情出现的表情大小
		}
								...
								html = html
										...
										.replace(
												...
												function($1) {
													...
													if (url) {
														return '<img style="width:22px" class="sina-emotion" src="'  //解析后图片的大小
																+ url
																+ '" alt="'
																+ $1 + '" />';
													}
